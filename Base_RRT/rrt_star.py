...
# -*- coding: utf-8 -*-
"""
RRT* (Goal-Bias + Narrow Passage Bias + Fixed Step + Adaptive Rewire) + Tkinter UI
- Input Gambar: warna:
    - #0000FF (biru)  : START
    - #FF0000 (merah) : GOAL
    - #000000 (hitam) : OBSTACLE
Left: gambar asli, Right: animasi RRT* + path
"""
import math
import random
import time
...
# -------------------- RRT* core (with Narrow Passage Bias + "Sakai-like") --------------------
...
class RRTStar:
    ...
    def plan(self, callback_draw_segment=None, callback_draw_best=None) -> Tuple[Optional[List[Tuple[int, int]]], int]:
        """
        Jalankan RRT*; kembalikan (path, iters)
        ...
        for iters in range(1, self.max_iter + 1):
            ...
            n = len(self.nodes) + 1
            radius = max(10.0, min(self.neighbor_radius,
            ...
            for j in neighbor_ids:
                ...
                if self._collision_free_segment((cand.x, cand.y), (new_x, new_y)):
                    cand_cost = self._cost(j) + math.hypot(new_x - cand.x, new_y - new_y + 0)  # keep clarity
            ...
            if self._collision_free_segment((new_x, new_y), (self.goal.x, self.goal.y)):
                ...
                while cur is not None:
                    nnode = self.nodes[cur]
                    path.append((nnode.x, nnode.y))
                    cur = nnode.parent
                path.reverse()
...
class RRTApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("RRT* (Narrow Passage + Fixed Step)")
